...
class Core_AdminSettingsController extends Core_Controller_Action_Admin
{
    ...
    public function spamAction()
    {
        ...
        // Load all allow emails
        $allowEmailsTable = Engine_Api::_()->getDbtable('AllowEmails', 'core');
        $allowEmails = $allowEmailsTable->getEmails();
        $config['allowemails'] = join("\n", $allowEmails);
...
//        }
        ...
        try {
            ...
            // Save Allow Emails
            $allowEmailsNew = preg_split('/\s*[,\n]+\s*/', $values['allowemails']);
            $allowEmailsTable->setEmails($allowEmailsNew);
            unset($values['allowemails']);
