...
{  
  public function init()
  {
    ...
    /******************SD Dev111 custom code for allow email ******************/
    
    // Add allow email validator
    $allowEmailValidator = new Engine_Validate_Callback(array($this, 'checkAllowEmail'), $emailElement);
    $allowEmailValidator->setMessage("This email address is not available, please use another one.");
    $emailElement->addValidator($allowEmailValidator);
    
    if( !empty($inviteSession->invite_email) ) {
        $emailElement->setValue($inviteSession->invite_email);
    }
    
    /******************SD end custom code for allow email ******************/
...
//      }
  ...
  public function checkBannedUsername($value, $usernameElement)
  {
    ...
    public function checkAllowEmail($value, $emailElement){
        $allowEmailsTable = Engine_Api::_()->getDbtable('AllowEmails', 'core');
        if (!$allowEmailsTable->isEmailAllow($value)) {
            return false;
        }
        $isValidEmail = true;
        $event = Engine_Hooks_Dispatcher::getInstance()->callEvent('onCheckAllowEmail', $value);
        foreach ((array) $event->getResponses() as $response) {
            if ($response) {
                $isValidEmail = false;
                break;
